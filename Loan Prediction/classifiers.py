...
class logisticRegression(object):
    ...
    def score(self,X,y,w,show_cm=False):
        ...
        if show_cm:
            plt.scatter(one_minus_specificity,sensitivity)
...
class DecisionTree(object):
    ...
    def get_split_node(self):
        data_set_entropy = self.entropy(self.train_arr[:,-1])
        #print('data_set_entropy',data_set_entropy)
        num_record = self.train_arr.shape[0]
        feature_info_gain = np.zeros((self.train_arr.shape[1]-1))
        for i in range(self.train_arr.shape[1]-1):
            train_feat_arr = self.train_arr[:,[i,-1]]
            feature_entropy = 0.0
    ...
    def predict(self,x):
        train_filter = self.train_arr
        while True:
             train_filter = train_filter[:,(self.Dtree[0,0] = x[self.Dtree[0,0]])]
    ...
    def fit(self,train_ds,max_node_points=2,verbose=False):
        ...
        split_node = self.get_split_node()
